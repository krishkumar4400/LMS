  ...
  return (
    <Link to={'/course/' + course._id} onClick={() => scrollTo(0,0)} className="flex flex-col hover:scale-105 transition-all duration-200 active:scale-95 border border-gray-500/40 pb-6 overflow-hidden rounded-lg">
      ...
      <div className="p-3 text-left ">
        ...
        <p className="text-sm font-medium text-gray-600">{course.educator.name}</p>
        <div className="flex items-center gap-2 space-x-2">
          <p>4.5</p>
          <div className="flex items-center">
            {[...Array(5)].map((_, i) => (
              <img src={assets.star} className="w-3.5 h-3.5 " alt="Star" key={i} />
            ))}
          </div>
          <p className="text-slate-900">22</p>
        </div>
